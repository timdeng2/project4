import json
from pathlib import Path

import fire
import matplotlib.pyplot as plt
import numpy as np
from PIL import Image, ImageDraw

import math
import os

# Define object type mapping
OBJECT_TYPES = {
    1: "Kart",
    2: "Track Boundary",
    3: "Track Element",
    4: "Special Element 1",
    5: "Special Element 2",
    6: "Special Element 3",
}

# Define colors for different object types (RGB format)
COLORS = {
    1: (0, 255, 0),  # Green for karts
    2: (255, 0, 0),  # Blue for track boundaries
    3: (0, 0, 255),  # Red for track elements
    4: (255, 255, 0),  # Cyan for special elements
    5: (255, 0, 255),  # Magenta for special elements
    6: (0, 255, 255),  # Yellow for special elements
}

# Original image dimensions for the bounding box coordinates
ORIGINAL_WIDTH = 600
ORIGINAL_HEIGHT = 400


def extract_frame_info(image_path: str) -> tuple[int, int]:
    """
    Extract frame ID and view index from image filename.

    Args:
        image_path: Path to the image file

    Returns:
        Tuple of (frame_id, view_index)
    """
    filename = Path(image_path).name
    # Format is typically: XXXXX_YY_im.png where XXXXX is frame_id and YY is view_index
    parts = filename.split("_")
    if len(parts) >= 2:
        frame_id = int(parts[0], 16)  # Convert hex to decimal
        view_index = int(parts[1])
        return frame_id, view_index
    return 0, 0  # Default values if parsing fails


def draw_detections(
    image_path: str, info_path: str, font_scale: float = 0.5, thickness: int = 1, min_box_size: int = 5
) -> np.ndarray:
    """
    Draw detection bounding boxes and labels on the image.

    Args:
        image_path: Path to the image file
        info_path: Path to the corresponding info.json file
        font_scale: Scale of the font for labels
        thickness: Thickness of the bounding box lines
        min_box_size: Minimum size for bounding boxes to be drawn

    Returns:
        The annotated image as a numpy array
    """
    # Read the image using PIL
    pil_image = Image.open(image_path)
    if pil_image is None:
        raise ValueError(f"Could not read image at {image_path}")

    # Get image dimensions
    img_width, img_height = pil_image.size

    # Create a drawing context
    draw = ImageDraw.Draw(pil_image)

    # Read the info.json file
    with open(info_path) as f:
        info = json.load(f)

    # Extract frame ID and view index from image filename
    _, view_index = extract_frame_info(image_path)

    # Get the correct detection frame based on view index
    if view_index < len(info["detections"]):
        frame_detections = info["detections"][view_index]
    else:
        print(f"Warning: View index {view_index} out of range for detections")
        return np.array(pil_image)

    # Calculate scaling factors
    scale_x = img_width / ORIGINAL_WIDTH
    scale_y = img_height / ORIGINAL_HEIGHT

    # Draw each detection

    nums_karts = 0

    for detection in frame_detections:
        class_id, track_id, x1, y1, x2, y2 = detection
        class_id = int(class_id)
        track_id = int(track_id)

        if class_id != 1:
            continue

        # Scale coordinates to fit the current image size
        x1_scaled = int(x1 * scale_x)
        y1_scaled = int(y1 * scale_y)
        x2_scaled = int(x2 * scale_x)
        y2_scaled = int(y2 * scale_y)

        # Skip if bounding box is too small
        if (x2_scaled - x1_scaled) < min_box_size or (y2_scaled - y1_scaled) < min_box_size:
            continue

        if x2_scaled < 0 or x1_scaled > img_width or y2_scaled < 0 or y1_scaled > img_height:
            continue
        nums_karts += 1
        print(f"--------{nums_karts}--------")


        img_x = img_width / 2
        img_y = img_height / 2
        print(f"Middle: {img_y} x {img_x}")
        center_x = x1_scaled + abs(x2_scaled - x1_scaled) / 2
        center_y = y1_scaled + abs(y2_scaled - y1_scaled) / 2
        print(f"Center: {center_y} x {center_x}")
        distance = math.sqrt((img_y - center_y)**2 + (img_x - center_x)**2)

        print(distance)
        # Get color for this object type
        if track_id == 0:
            color = (255, 0, 0)
            print("Nominal Ego")
        else:
            color = COLORS.get(class_id, (255, 255, 255))
            print("Normal")

        # Draw bounding box using PIL
        print("drawing rectangle")
        draw.rectangle([(x1_scaled, y1_scaled), (x2_scaled, y2_scaled)], outline=color, width=thickness)

    # Convert PIL image to numpy array for matplotlib
    return np.array(pil_image)


def extract_kart_objects(
    info_path: str, view_index: int, img_width: int = 150, img_height: int = 100, min_box_size: int = 5
) -> list:
    """
    Extract kart objects from the info.json file, including their center points and identify the center kart.
    Filters out karts that are out of sight (outside the image boundaries).

    Args:
        info_path: Path to the corresponding info.json file
        view_index: Index of the view to analyze
        img_width: Width of the image (default: 150)
        img_height: Height of the image (default: 100)

    Returns:
        List of kart objects, each containing:
        - instance_id: The track ID of the kart
        - kart_name: The name of the kart
        - center: (x, y) coordinates of the kart's center
        - is_center_kart: Boolean indicating if this is the kart closest to image center
    """
    info_path = Path(info_path)
    base_name = info_path.stem.replace("_info", "")
    image_file = list(info_path.parent.glob(f"{base_name}_{view_index:02d}_im.jpg"))[0]
    pil_image = Image.open(image_file)
    if pil_image is None:
        print(f"Skipped Image {image_file}")
        return []

    img_width, img_height = pil_image.size
    with open(info_path) as f:
        info = json.load(f)

    if view_index < len(info["detections"]):
        frame_detections = info["detections"][view_index]
    else:
        print(f"Warning: View index {view_index} out of range for detections")
        return []
    
    scale_x = img_width / ORIGINAL_WIDTH
    scale_y = img_height / ORIGINAL_HEIGHT

    img_center_x = img_width / 2
    img_center_y = img_height / 2

    out = []


    for detection in frame_detections:
        kart_info = {"instance_id" : -1, "kart_name" : "steve", "center" : (-1, -1), "is_center_kart" : False}
        class_id, track_id, x1, y1, x2, y2 = detection
        class_id = int(class_id)
        track_id = int(track_id)

        if class_id != 1:
            continue

        if track_id >= len(info["karts"]):
            continue

        # Scale coordinates to fit the current image size
        x1_scaled = int(x1 * scale_x)
        y1_scaled = int(y1 * scale_y)
        x2_scaled = int(x2 * scale_x)
        y2_scaled = int(y2 * scale_y)

        # Skip if bounding box is too small or OOB
        if (x2_scaled - x1_scaled) < min_box_size or (y2_scaled - y1_scaled) < min_box_size:
            continue

        if x2_scaled < 0 or x1_scaled > img_width or y2_scaled < 0 or y1_scaled > img_height:
            continue

        # assumes the 1 coordinate is < 2 coordinate
        kart_center_x = x1_scaled + (abs(x2_scaled - x1_scaled) / 2)
        kart_center_y = y1_scaled + (abs(y2_scaled - y1_scaled) / 2)

        kart_info["center"] = (kart_center_x, kart_center_y)
        kart_info["instance_id"] = track_id
        kart_info["kart_name"] = info["karts"][track_id]

        out.append(kart_info)

    min_distance = float("inf")
    min_idx = -1

    for i, kart in enumerate(out):
        # compute distance
        center_x = kart["center"][0]
        center_y = kart["center"][1]
        distance = math.sqrt((center_x - img_center_x)**2 + (center_y - img_center_y)**2)

        if distance < min_distance:
            min_idx = i
            min_distance = distance

    if min_idx != -1:
        out[min_idx]["is_center_kart"] = True
        

    return out


def extract_track_info(info_path: str) -> str:
    """
    Extract track information from the info.json file.

    Args:
        info_path: Path to the info.json file

    Returns:
        Track name as a string
    """

    with open(info_path) as f:
        info = json.load(f)

    return info.get("track", "None")
 

def generate_qa_pairs(info_path: str, view_index: int, img_width: int = 150, img_height: int = 100) -> list:
    """
    Generate question-answer pairs for a given view.

    Args:
        info_path: Path to the info.json file
        view_index: Index of the view to analyze
        img_width: Width of the image (default: 150)
        img_height: Height of the image (default: 100)

    Returns:
        List of dictionaries, each containing a question and answer
    """
    karts = extract_kart_objects(info_path, view_index)
    info_path_obj = Path(info_path)
    image_file = f"{info_path_obj.parent.name}/{info_path_obj.stem.replace('_info', '')}_{view_index:02d}_im.jpg"

    track = extract_track_info(info_path)

    out = []

    if len(karts) == 0:
        return []

    center_kart_idx = -1
    center_kart_coords = (-1,-1)

    for i, kart in enumerate(karts):
        # 1. Ego car question 
        # What kart is the ego car?
        if kart['is_center_kart']:
            out.append(
                {
                    "question": "What kart is the ego car?",
                    "answer": kart['kart_name'],
                    "image_file": image_file
                }
            )
            center_kart_idx = i
            center_kart_coords = kart['center']
    
    if center_kart_idx == -1:
        print("Could not find ego kart")
        return []
    
    # 2. Total karts question
    # How many karts are there in the scenario?
    out.append(
        {
            "question": "How many karts are there in the scenario?",
            "answer": str(len(karts)),
            "image_file": image_file          
        }
    )

    # 3. Track information questions
    # What track is this?
    out.append(
        {
            "question": "What track is this?",
            "answer": track,
            "image_file": image_file          
        }
    )
        
    karts_left = 0
    karts_right = 0
    karts_front = 0
    karts_behind = 0
    # 4. Relative position questions for each kart
    # Is {kart_name} to the left or right of the ego car?
    # Is {kart_name} in front of or behind the ego car?
    # Where is {kart_name} relative to the ego car?
    for i, kart in enumerate(karts):
        if i == center_kart_idx:
            continue

        x, y = kart['center']
        # sides are -1 or 1 indicating left, same, or right of center kart. If same, do nothing
        x_side = 0
        # sides are -1 or 1 indicating behind, same, or front of center kart. If same, do nothing
        y_side = 0
        if x <= center_kart_coords[0]:
            x_side = -1
            karts_left += 1

        if x > center_kart_coords[0]:
            x_side = 1
            karts_right += 1

        if y >= center_kart_coords[1]:
            y_side = -1
            karts_behind += 1

        if y < center_kart_coords[1]:
            y_side = 1
            karts_front += 1

        if x_side != 0:
            horizontal = "left" if x_side == -1 else "right"
            out.append(
                {
                    "question": f"Is {kart['kart_name']} to the left or right of the ego car?",
                    "answer": horizontal,
                    "image_file": image_file                  
                }
            )

        if y_side != 0:
            vertical = "back" if y_side == -1 else "front"
            out.append(
                {
                    "question": f"Is {kart['kart_name']} in front of or behind the ego car?",
                    "answer": vertical,
                    "image_file": image_file                  
                }
            )

        if x_side != 0 and y_side != 0:
            horizontal = "left" if x_side == -1 else "right"
            vertical = "back" if y_side == -1 else "front"
            out.append(
                {
                    "question": f"Where is {kart['kart_name']} relative to the ego car?",
                    "answer": f"{vertical} and {horizontal}",
                    "image_file": image_file                  
                }
            )

    # 5. Counting questions
    # How many karts are to the left of the ego car?
    if karts_left != 0:
        out.append(
            {
                "question": "How many karts are to the left of the ego car?",
                "answer": str(karts_left),
                "image_file": image_file                   
            }
        )
    # How many karts are to the right of the ego car?
    if karts_right != 0:
        out.append(
            {
                "question": "How many karts are to the right of the ego car?",
                "answer": str(karts_right),
                "image_file": image_file                   
            }
        )
    # How many karts are in front of the ego car?
    if karts_front != 0:
        out.append(
            {
                "question": "How many karts are in front of the ego car?",
                "answer": str(karts_front),
                "image_file": image_file                   
            }
        )
    # How many karts are behind the ego car?
    if karts_behind != 0:
        out.append(
            {
                "question": "How many karts are behind the ego car?",
                "answer": str(karts_behind),
                "image_file": image_file                   
            }
        )

    return out

def generate_all_qa_pairs(info_dir: str, output_file: str):
    info_dir = Path(info_dir)
    all_qa_pairs = []

    info_files = list(info_dir.glob("*_info.json"))
    print(f"{len(info_files)} total info files")

    for i, info_file in enumerate(info_files):

        with open(info_file) as f:
            info = json.load(f)

        num_views = len(info.get("detections", []))

        for view_index in range(num_views):
            base_name = info_file.stem.replace("_info", "")
            image_files = list(info_dir.glob(f"{base_name}_{view_index:02d}_im.jpg"))
            if not image_files:
                print(f"Skipping view {base_name}_{view_index:02d}_im.jpg — no image found")
                continue

            try:
                qa_pairs = generate_qa_pairs(str(info_file), view_index)
                all_qa_pairs.extend(qa_pairs)
            except Exception as e:
                print(f"Error processing view {base_name}_{view_index:02d}: {e}")
                continue

        if i % 25 == 0:
            print(f"Processed {i} info_files / {len(info_files)}")

    print(f"\nTotal QA pairs generated: {len(all_qa_pairs)}")

    with open(output_file, "w") as f:
        json.dump(all_qa_pairs, f, indent=2)

    print(f"Saved to {output_file}")

def check_qa_pairs(info_file: str, view_index: int):
    """
    Check QA pairs for a specific info file and view index.

    Args:
        info_file: Path to the info.json file
        view_index: Index of the view to analyze
    """
    # Find corresponding image file
    info_path = Path(info_file)
    base_name = info_path.stem.replace("_info", "")
    image_file = list(info_path.parent.glob(f"{base_name}_{view_index:02d}_im.jpg"))[0]

    # Visualize detections
    annotated_image = draw_detections(str(image_file), info_file)

    # Display the image
    plt.figure(figsize=(12, 8))
    plt.imshow(annotated_image)
    plt.axis("off")
    plt.title(f"Frame {extract_frame_info(str(image_file))[0]}, View {view_index}")
    plt.show()

    # Generate QA pairs
    qa_pairs = generate_qa_pairs(info_file, view_index)

    # Print QA pairs
    print("\nQuestion-Answer Pairs:")
    print("-" * 50)
    for qa in qa_pairs:
        print(f"Q: {qa['question']}")
        print(f"A: {qa['answer']}")
        print("-" * 50)


"""
Usage Example: Visualize QA pairs for a specific file and view:
   python generate_qa.py check --info_file ../data/valid/00000_info.json --view_index 0

You probably need to add additional commands to Fire below.
"""


def main():
    fire.Fire({"check": check_qa_pairs, "generate_all": generate_all_qa_pairs})


if __name__ == "__main__":
    main()
